...
The dimmer and target highlight use `pointer-events: none`; the real app stays interactive. The control bar alone uses `pointer-events: auto`. If an exact target is outside the viewport, the overlay scrolls it into view before positioning the tooltip beside it. Restart navigates to `startPath`, or hard-reloads it when it is already there, so ordinary in-memory React state starts fresh. DemoFlow never attempts to mutate arbitrary server, account, or persisted application state; apps that need that require an explicitly safe local fixture/reset path.

The overlay listens for `click`, `input`, `submit`, `popstate`, and URL changes. A label target is resolved to its associated input, textarea, select, or editable control when one exists, so `input-and-click` steps highlight and observe the actual interactive field. Before positioning, it scrolls a target outside the central 35–65% vertical action zone into the viewport center—even when that target is technically visible near the bottom—then hides its own marker, waits for two stable animation frames (with a short bounded fallback), and positions the tooltip. Its placement engine scores the available above, below, and side positions against visible host-page controls, headings, labels, and text; it preserves the highlighted action first, then chooses the placement that obscures the least nearby content. In a constrained viewport, it can use a compact side card rather than covering the form or heading with a full-width tooltip. The real button or input stays visible and clickable. It recomputes those fixed overlay coordinates on host-page scroll, visual-viewport changes, target resize, and React DOM mutations, so the marker follows a target through layout changes. It resolves the next step after the configured `advance` condition becomes true. For a next-step target that is not yet mounted, it shows a neutral waiting state and retries after DOM mutations and on a short timer for up to five seconds. Only then does it show and report a target failure. This supports React and Next.js conditional rendering, client transitions, and asynchronous state updates without treating them as immediate errors. After the final advance condition, it shows a Demo complete panel with Restart and Close controls; it never silently removes the walkthrough.
